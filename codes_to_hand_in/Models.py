...
def output_size(input_shape, kernel_size,padding = 0, dilation = 1, stride = 1):
    '''
    calculate output size of a given dimension based on properties of current layer

    Parameters
    ----------
    input_shape : int
        shape of input (only this dimension)
    kernel_size : int
        kernel size for this dimension
    padding : int, optional
        padding of layer. The default is 0.
    dilation : int, optional
        dilation of layer. The default is 1.
    stride : int, optional
        stride of layer. The default is 1.
...
def CNN_output_shape(input_dimension = (128,63),
    ...
    input_dimension : int, optional
        2 dimensional input dimension (row, column). The default is (128,63).
    kernel : int, optional
        2d kernel size. The default is (2,3).
    layers : int, optional
        number of CNN layers. The default is 2.
    max_pooling : boolean, optional
        whether it contains max pooling layers. The default is True.
    max_kernel : tuple, optional
        max pooling kernel size. The default is (1,2).
